...
async function claimCardsRefresh(){
	...
	if(MyLibrary.claimsource ==2){//share lease
		...
		$('#sect_claims_w').empty().append(leaseAmount+' Tokens');
	...
	if(MyLibrary.claimsource ==3){//staked
		...
		$('#sect_claims_w').empty().append(holdings+' TOKENS');
		...
		if(holdings == 0){return;}else{
			...
			//Rewards Due
			try{
				tokenInst.methods.checkStakeReflection().call({from: MyLibrary.wallet}).then(function (result,error) {
					var reward_due = fromWeiToFixed8(result);
					$('#sect_due_w').empty().append(reward_due + ' BNB');
				});
			}catch(error) {
...
tokenInst.events.ClaimReflectionLease()
	.on('data', async function(event){
		...
		var nonTxAction = receivedETH+' bnb, from: '+privatize;
...
tokenInst.events.ClaimReflectionStake()
	.on('data', async function(event){
		...
		var nonTxAction = receivedETH+' bnb';
...
async function port_staticView(){
	...
	try{  
		...
		if(luckyDip == true){
			...
			$('#mbcard_tokens').empty().append('minimum buy: <p class="markgreen">' + luckyDipMin + '</p> BRAAVOS');
		}else{
	...
	try{
		...
		if(eligibility == true){
			...
			var reimbursementStatus = '<p>Buy Tax Reimburse: </p><p class="markgreen">True</p>';
			var rewardsStatus = '<p>Reward Claimable: </p><p class="markgreen">True</p>';
			...
			var taxStatus = '<p class="markred">Sell Tax: '+selltax+'%</p>';
			var reimbursementStatus = '<p>Buy Tax Reimburse: </p><p class="markred">False</p>';
			var rewardsStatus = '<p>Reward Claimable: </p><p class="markred">False</p>';
		}
		//output
		$('#mbcard_taxstatus').empty().append(taxStatus);
		$('#mbcard_forefeits').empty().append(reimbursementStatus + rewardsStatus);
	}catch(error) {
		console.log(error); 
	}
