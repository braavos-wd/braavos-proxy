...
async function playProfile(){
	...
	if(eligibility == true){
		var taxStatus = '<p class="markgreen">Sell Tax: 8%</p>';		
...
async function playRussianRoulette(){
	//estimate gasLimit
	var encodedData = tokenInst.methods.playRussianRoulette().encodeABI();
	...
	playEffects();
	//transaction
	tokenInst.methods.playRussianRoulette().send({
		from: MyLibrary.wallet,
		gasPrice: gasPrice,
		gasLimit: estimateGas //actual gas is being underestimated for deeper state changes in other dependency functions
	})
